Component({
  ...
  findPrizeIndex(name) {
    ...
    async start() {
      ...
      let activeIndex = +this.props.currentIndex || 0;
      // 奖品项下标
    /*  var prizeIndex = this.findPrizeIndex(this.props.prizeName);
      ...
      let resutl = await this.props.onStart();
      console.log("onStart result",resutl,+Date.now())
      if(resutl) {
      let  {prizeName,success} = resutl
      if(!success) {
          this.resultFail = true
          return 
      }
      let prizeIndex = this.findPrizeIndex(prizeName);
      if (prizeIndex === -1) {
         this.resultFail = true
         return
      }
      ...
      this.currentStep = 0;   
      this.gotResult = true
      console.log("gotResult",{totalSteps:this.totalSteps,activeIndex,prizeIndex,prizeName},+Date.now())
      }
    },
    done(activeIndex) {
      var _this2 = this;
      ...
      setTimeout(function () {
       ...
       console.log("done",{activeIndex,prizeName:this.props.prizeName},+Date.now())
      this.props.onFinish({activeIndex,prizeName: this.props.prizeName,resultFail: this.resultFail});
    }
  }
});